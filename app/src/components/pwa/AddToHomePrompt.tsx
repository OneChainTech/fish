...
export function AddToHomePrompt() {
  const { isVisible, dismiss } = usePwaInstallPrompt();
    ...
    <div className="pointer-events-auto fixed inset-x-4 bottom-32 z-[60] rounded-2xl border border-sky-100 bg-white/95 p-4 shadow-lg backdrop-blur-md transition-all duration-300 md:inset-auto md:bottom-12 md:right-8 md:w-80">
      <div className="relative flex items-start gap-3">
        <button
          type="button"
          onClick={handleClose}
          className="absolute -right-2 -top-2 inline-flex h-6 w-6 items-center justify-center rounded-full bg-slate-100 text-xs font-semibold text-slate-500 shadow-sm ring-1 ring-slate-200/60 transition hover:bg-slate-200"
          aria-label="关闭添加到主屏幕提示"
        >
          ×
        </button>
        <div className="flex-1">
          <p className="text-sm font-medium text-slate-800">将应用添加到主屏幕</p>
          <div className="mt-3 space-y-3">
            <div className="rounded-xl bg-sky-50/70 px-3 py-2 text-xs leading-5 text-sky-700">
              ...
              点击右上角分享按钮，选择“添加到主屏幕”，按照提示确认即可完成安装。
            </div>
